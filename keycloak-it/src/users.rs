...
//! scimitar's parsing/validation/PATCH code, not to be a usable SCIM server.
...
use scimitar::user::{USER_SCHEMA_URI, User, user_schema};
...
pub async fn list(State(state): State<AppState>) -> Json<Value> {
    ...
    // scimitar itself (see src/filter.rs's module doc) and isn't exercised by the
